...
- **Option 1: Astro Native i18n:**
  - _Pros:_ Out-of-the-box support for route prefixes, language switching utilities, and automated fallback configurations. Extremely easy to configure in `astro.config.mjs`.
  - _Cons:_ Less control over manual locale rewrite mapping (unnecessary for our use case).
- **Option 2: Custom Route Parameters:**
  - _Pros:_ Custom redirects and URL rewrites directly mapped in folder routes.
  - _Cons:_ Significant boilerplate code, need to manually inject `locale` into links (`/[locale]/about`), prone to routing errors.
